...
Creates synthetic transactions with known fraud patterns injected directly
into the existing SQLite database, then wraps them as CaseRecords for
evaluation. These cases are designed to test agent generalization —
the patterns are real but the specific transactions are fabricated,
so the agent cannot rely on memorized data.
...
Output
------
    implementations/fraud_analytics/data/synthetic_cases.jsonl
    (also appends synthetic transactions to fraud_transactions.db)

Fraud patterns covered (one scenario per pattern)
--------------------------------------------------
    1.  card_not_present    — online purchase, no chip, new merchant, high amount
    2.  unusual_velocity    — 8 small transactions in 90 minutes on same card
    3.  account_takeover    — bad PIN attempts then large cash withdrawal
    4.  identity_theft      — new merchant category never seen by this client
    5.  smurfing            — 6 transactions just under $500 reporting threshold
    6.  merchant_fraud      — unknown merchant ID, mismatched MCC
    7.  geo_anomaly         — transactions in two cities 3000 km apart within 2 hours
    8.  unknown             — multiple weak signals, no single dominant pattern

Plus 4 legitimate edge cases designed to fool the agent:
    9.  legit_high_amount   — large but explainable one-off purchase (flight)
    10. legit_new_merchant  — first visit to a new but normal merchant
    11. legit_velocity      — many small transactions at a supermarket (groceries)
    12. legit_online        — online purchase consistent with client's usual behaviour
"""
...
# Synthetic client/card IDs — use high numbers to avoid clashing with real data
SYN_PREFIX  = "SYN"
...
def insert_transactions(conn: sqlite3.Connection, rows: list[dict]) -> None:
    """Insert synthetic transaction rows into the transactions table."""
    conn.executemany("""
...
def insert_card(conn: sqlite3.Connection, card: dict) -> None:
    conn.execute("""
...
def base_user(cid: int) -> dict:
    return {
        "client_id": client_id(cid),
        "current_age": 35, "retirement_age": 65,
        "birth_year": 1989, "birth_month": 6,
        "gender": "M", "address": "123 Test St",
        "latitude": 40.7128, "longitude": -74.0060,
        "per_capita_income": 45000, "yearly_income": 75000,
        "total_debt": 12000, "credit_score": 720,
        "num_credit_cards": 2,
    }


def base_card(cid: int) -> dict:
    return {
        "card_id": card_id(cid),
        "client_id": client_id(cid),
        "card_brand": "Visa", "card_type": "Credit",
        "card_number": f"4111{cid:012d}",
        "expires": "12/2027", "cvv": "123",
        "has_chip": "YES", "num_cards_issued": 1,
        "credit_limit": 10000,
        "acct_open_date": "2018-01-15",
        "year_pin_last_changed": 2022,
        "card_on_dark_web": "No",
    }


def normal_history(cid: int, anchor: datetime, n: int = 15) -> list[dict]:
    """Generate n normal transactions in the 30 days before anchor."""
    rows = []
    for i in range(n):
...
def scenario_card_not_present(conn, cid, anchor):
    """Fraud: Online purchase, no chip, new merchant, amount far above average."""
    history = normal_history(cid, anchor)  # avg ~$60
    seed_txn = txn_id()
    seed = {
        ...
        "merchant_city": "Online", "merchant_state": "CA",
        "zip": "90210", "mcc": 5732,
        "errors": None, "is_fraud": 1,
        "date": anchor.strftime("%Y-%m-%d %H:%M:%S"),
    }
...
def scenario_unusual_velocity(conn, cid, anchor):
    """Fraud: 8 transactions in 90 minutes, small amounts, same card."""
...
def scenario_account_takeover(conn, cid, anchor):
    """Fraud: 3 bad PIN attempts then large withdrawal at ATM."""
    history = normal_history(cid, anchor)
    seed_txn = txn_id()
    error_txns = []
    for i in range(3):
...
def scenario_identity_theft(conn, cid, anchor):
    """Fraud: Purchase in MCC the client has never used (jewellery), high amount."""
    history = normal_history(cid, anchor)  # all grocery/restaurant
    seed_txn = txn_id()
    seed = {
        "transaction_id": seed_txn,
        "client_id": client_id(cid),
        ...
        "merchant_city": "Beverly Hills", "merchant_state": "CA",
        "zip": "90210", "mcc": 5944,
        "errors": None, "is_fraud": 1,
        "date": anchor.strftime("%Y-%m-%d %H:%M:%S"),
    }
...
def scenario_smurfing(conn, cid, anchor):
    """Fraud: 6 transactions of $490-$498, just under $500 reporting threshold."""
    history = normal_history(cid, anchor)
    txns = []
    seed_txn = txn_id()
    ...
    for i, amt in enumerate(amounts):
        ...
        txns.append({
            ...
            "merchant_city": "Online", "merchant_state": "TX",
            "zip": "75001", "mcc": 6051,
            "errors": None, "is_fraud": 1,
...
def scenario_merchant_fraud(conn, cid, anchor):
    """Fraud: Completely unknown merchant ID, MCC 5999 (misc retail) with high amount."""
    history = normal_history(cid, anchor)
    seed_txn = txn_id()
    seed = {
        "transaction_id": seed_txn,
        ...
        "merchant_city": "Las Vegas", "merchant_state": "NV",
        "zip": "89101", "mcc": 5999,
        "errors": None, "is_fraud": 1,
        "date": anchor.strftime("%Y-%m-%d %H:%M:%S"),
    }
...
def scenario_geo_anomaly(conn, cid, anchor):
    """Fraud: Transaction in NYC then London 2 hours later (impossible travel)."""
    history = normal_history(cid, anchor)
    # First transaction in NYC (normal)
    nyc_txn = {
        "transaction_id": txn_id(),
        "client_id": client_id(cid),
        "card_id": card_id(cid),
    ...
    seed = {
        ...
        "merchant_city": "London", "merchant_state": "ENG",
        "zip": "EC1A", "mcc": 7011,
        "errors": None, "is_fraud": 1,
        "date": anchor.strftime("%Y-%m-%d %H:%M:%S"),
    }
...
def scenario_unknown_pattern(conn, cid, anchor):
    """Fraud: Multiple weak signals, no dominant pattern. Agent must use 'unknown'."""
    history = normal_history(cid, anchor)
    seed_txn = txn_id()
    # Slightly above average, online, familiar MCC but different city
    seed = {
        "transaction_id": seed_txn,
        "client_id": client_id(cid),
        "card_id": card_id(cid),
        ...
        "merchant_city": "Miami", "merchant_state": "FL",
        "zip": "33101", "mcc": 5411,
        "errors": None, "is_fraud": 1,
        "date": anchor.strftime("%Y-%m-%d %H:%M:%S"),
    }
    # Add a declined transaction just before
    declined = {
        "transaction_id": txn_id(),
        "client_id": client_id(cid),
        "card_id": card_id(cid),
...
def scenario_legit_high_amount(conn, cid, anchor):
    """Legit: Large purchase but client has a history of large purchases."""
    # History with several large transactions
    history = []
    for i in range(10):
        history.append({
            "transaction_id": txn_id(),
            "client_id": client_id(cid),
            "card_id": card_id(cid),
            "amount": round(400 + (i % 4) * 200 + 0.99, 2),
            "use_chip": "Chip Transaction",
...
def scenario_legit_new_merchant(conn, cid, anchor):
    """Legit: First visit to a new merchant but amount and MCC are normal."""
    history = normal_history(cid, anchor)
    seed_txn = txn_id()
    seed = {
        "transaction_id": seed_txn,
...
def scenario_legit_velocity(conn, cid, anchor):
    """Legit: Many transactions in short time — client went grocery shopping."""
    history = normal_history(cid, anchor)
    txns = []
    seed_txn = txn_id()
    # 5 transactions over 2 hours — below the 5-in-2hr fraud threshold
    for i in range(5):
        tid = seed_txn if i == 0 else txn_id()
        txns.append({
            "transaction_id": tid,
...
def scenario_legit_online(conn, cid, anchor):
    """Legit: Online purchase but client regularly shops online in same category."""
    # History includes several online transactions
    history = []
    for i in range(12):
        history.append({
            "transaction_id": txn_id(),
...
SCENARIOS = [
    # (function, client_num, label, description)
...
def main() -> None:
    if not DB_PATH.exists():
        ...
        print("   Run create-db first.")
        return

    conn = sqlite3.connect(DB_PATH)
    anchor = datetime(2024, 3, 15, 14, 30, 0)  # fixed anchor for reproducibility
    ...
    for i, (fn, cid, description) in enumerate(SCENARIOS, start=1):
        ...
        insert_user(conn, base_user(cid))
        insert_card(conn, base_card(cid))
